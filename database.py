...
class Database:
    ...
    async def create_or_get_user(self, telegram_id: int, username: str = None) -> int:
        ...
        async with self.pool.acquire() as conn:
            ...
            user_id = await conn.fetchval(
                "INSERT INTO users (telegram_id, username) VALUES ($1, $2) RETURNING id", telegram_id, username
            )
            return user_id
    ...
    async def create_account(self, user_id: int, name: str) -> bool:
        ...
        try:
            async with self.pool.acquire() as conn:
                await conn.execute("INSERT INTO accounts (name, owner_id) VALUES ($1, $2)", name, user_id)
                return True
    ...
    async def get_category_by_name(self, name: str) -> Optional[int]:
        ...
        async with self.pool.acquire() as conn:
            result = await conn.fetchrow("SELECT id FROM categories WHERE name ILIKE $1", name)
        ...
        try:
            async with self.pool.acquire() as conn:
                ...
                owner = await conn.fetchrow(
                    "SELECT id FROM accounts WHERE id = $1 AND owner_id = $2", account_id, owner_id
                )
                if not owner:
                    return False
    ...
    async def get_stats(self, user_id: int, period_days: int) -> Dict[str, Any]:
        ...
        async with self.pool.acquire() as conn:
            ...
            for cat in categories:
                ...
                category_stats.append({"name": cat["name"], "amount": amount, "percentage": percentage})
